...
our ($queryfile, $dbloc);
our ($outfile, $buildver, $step, $checkfile, $remove, $verdbsnp, $ver1000g, $genetype, $veresp);
...
defined $verdbsnp or $verdbsnp = 137;  ## updated dbsnp version !!
$genetype ||= 'refgene';
...
$checkfile; #and checkFileExistence ();  ## checking files is switched off !! For only genomic annotation we dont need to make sure other files are present or not !!  
...
while (<FUNCTION>) {
	...
	### replace ',' with ';' in function and gene fields
	...
	if (not @{$allstep[1]}) {
		if ($valistep{1}) {
			if (defined ($_ = <STEP1>)) {
				...
				### replace ',' with ';' in exonic function and amino acid change fields
